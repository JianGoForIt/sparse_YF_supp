...
def train_data(mini_batch, feature_batch, targets, word_attn_model, mix_softmax, optimizer, criterion):
...
#     print "~~~~", s.size(), feature_batch.size()
...
#         print token
        token = [_.cuda() for _ in pad_batch(token)]
#         print len(token), token[0].size(), token[1].size()
        yield token, Variable(torch.from_numpy(feature)).cuda() , Variable(torch.FloatTensor(label), requires_grad= False).cuda()
...
#         print token
        token = [_.cuda() for _ in pad_batch(token)]
        yield token, Variable(torch.from_numpy(feature)).cuda()
